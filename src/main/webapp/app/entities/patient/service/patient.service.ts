...
type RestOf<T extends IPatient | NewPatient> = Omit<T, 'dateBirth'> & {
...
export class PatientService {
  ...
  protected convertDateFromClient<T extends IPatient | NewPatient | PartialUpdatePatient>(patient: T): RestOf<T> {
    return {
      ...patient,
      dateBirth: patient.dateBirth?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPatient: RestPatient): IPatient {
    return {
      ...restPatient,
      dateBirth: restPatient.dateBirth ? dayjs(restPatient.dateBirth) : undefined,
    };
  }
